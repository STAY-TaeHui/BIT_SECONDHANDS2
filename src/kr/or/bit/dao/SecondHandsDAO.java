...
public class SecondHandsDAO {
	 ...
	 * JSONObject obj = new JSONObject();
	 * 
	 * obj.put("pimg_name", rs.getString("pimg_name")); obj.put("p_subj",
	 * rs.getString("p_subj")); obj.put("p_price", rs.getInt("p_price"));
	 * obj.put("p_wr_time", rs.getString("p_wr_time").substring(0, 11));
	 * obj.put("p_num", rs.getInt("p_num")); obj.put("storename",
	...
	public JSONArray searchProduct(String keyword) {
		...
		try {
			...
			String sql = "select pi.pimg_name, p.p_subj,p.p_price, p.p_wr_time,p.p_num, p.storename, p.p_status"
					+ "	from product p left join product_img pi  on p.p_num=pi.p_num"
					+ " where (p.p_status=1 and pi.pimg_num=1) and (p.p_subj Like '%" + keyword
					+ "%' or p.p_content Like '%" + keyword + "%')";
			pstmt = conn.prepareStatement(sql);
	...
	public JSONArray ProductOderByTime(String keyword) {
		...
		try {
			...
			String sql = "select pi.pimg_name, p.p_subj,p.p_price, p.p_wr_time,p.p_num, p.storename, p.p_status"
					+ "	from product p left join product_img pi  on p.p_num=pi.p_num"
					+ " where (p.p_status=1 and pi.pimg_num=1) and (p.p_subj Like '%" + keyword
					+ "%' or p.p_content Like '%" + keyword + "%') " + "order by p.p_wr_time desc";
			pstmt = conn.prepareStatement(sql);
	...
	public JSONArray ProductOderByPrice(String keyword) {
		...
		try {
			...
			String sql = "select pi.pimg_name, p.p_subj,p.p_price, p.p_wr_time,p.p_num, p.storename, p.p_status"
					+ "	from product p left join product_img pi  on p.p_num=pi.p_num"
					+ " where (p.p_status=1 and pi.pimg_num=1) and (p.p_subj Like '%" + keyword
					+ "%' or p.p_content Like '%" + keyword + "%') " + "order by p.p_price";
			pstmt = conn.prepareStatement(sql);
...
//		         System.out.println(obj.toString());
	...
	public JSONArray C_ProductOder(String keyword, String type) {
		...
		try {
			...
			if (type.equals("time")) {
				...
				sql += "select pi.pimg_name, p.p_subj,p.p_price, p.p_wr_time, p.p_num, p.storename "
						+ "from product p left join product_img pi " + "on p.p_num=pi.p_num "
						+ "where (p.p_status=1 and pi.pimg_num=1) and p.b_num=" + c_num + "order by p.p_price";
			}
	...
	public boolean productImgEdit(String image_name, String p_num, int pimg_num) {
		...
		try {
			...
			sql += "Update product_img set pimg_name='" + image_name + "', pimg_size=100  where pimg_num=" + pimg_num
					+ " and p_num=" + p_num;
