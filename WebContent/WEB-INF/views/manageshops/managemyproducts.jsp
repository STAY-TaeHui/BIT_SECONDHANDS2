...
<body>
						...
						<ul id="filter_menu">
							<li>
								<div class="input-group" id="search_myproducts">
									<input type="text" id="mykeyword" name="mykeyword"
										onkeyup="if(window.event.keyCode==13){searchmyp()}"
										placeholder="내 상품 검색">
									<button type="button" onclick="searchmyp()"
										id="searchmyproductbtn">
										<i class="ti-search"></i>
									</button>
								</div>
							</li>
	...
	-->
							<li><select id="status" onchange="productlistByStaus()">
									...
									<tbody id="tbody">
										...
										<c:choose>
											<c:when test="${not empty myproducts}">
												<c:forEach items="${myproducts}" var="myproducts" begin="0"
													varStatus="status" end="${fn:length(myproducts)}">
													<tr class="product">
														<td class="p_img"><a
															href='productdetail.do?p_num=${myproducts.p_num}&storename=${sessionScope.storename}'>
																<img
																src="${pageContext.request.contextPath}/fileUpload/${myproducts.pimg_name}" />
														</a></td>
														<td class="p_name"><p class="sbj">${myproducts.p_subj}</p></td>
														<td class="p_status"><c:choose>
														...
														<td class="p_fnc" id="${myproducts.p_num}"><input
															type="button" value="수정" name="edit_p" class="editbtn"
															onclick="edit_p(${myproducts.p_num})"> <input
															type="button" value="삭제" name="delte_p" class="deletebtn"
															onclick="delete_p(${myproducts.p_num})"></td>
